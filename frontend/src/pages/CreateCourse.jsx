...
const CreateCourse = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/courses', courseData, config);
      const courseId = res.data.data._id;

      // Save quizzes sequentially
      for (const quiz of quizzes) {
        if (quiz.title && quiz.questions.length > 0) {
          await axios.post(`http://localhost:5000/api/courses/${courseId}/quizzes`, quiz, config);
        }
      }
