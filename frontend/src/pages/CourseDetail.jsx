...
const CourseDetail = () => {
  ...
  const handleResourceComplete = async (resourceId) => {
    if (user && user.role === 'Student' && isEnrolled) {
      ...
      try {
        ...
        await axios.post(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/progress/${id}/resource/${resourceId}/complete`, {}, config);
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/courses/${id}`);
    ...
    const fetchQuizzes = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/courses/${id}/quizzes`);
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/courses/${id}/enroll`, {}, config);
      setIsEnrolled(true);
              ...
              {course.introVideoUrl ? (
                <div className="w-full h-full flex items-center justify-center aspect-w-16 aspect-h-9">
                  {getEmbedUrl(course.introVideoUrl) ? (
                    ...
                    <ReactPlayer 
                      url={course.introVideoType === 'local-video' ? `${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}${course.introVideoUrl}` : course.introVideoUrl}
                      width="100%"
                      height="100%"
                      controls={true}
                    />
                    ...
                    {(!activeResource.type || activeResource.type === 'video-link' || activeResource.type === 'local-video') ? (
                      (activeResource.videoUrl || activeResource.fileUrl) ? (
                        getEmbedUrl(activeResource.videoUrl) ? (
                          ...
                          <ReactPlayer 
                            url={activeResource.type === 'local-video' ? `${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}${activeResource.fileUrl}` : activeResource.videoUrl}
                            width="100%"
                      ...
                      activeResource.fileUrl ? (
                        <iframe 
                          src={`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}${activeResource.fileUrl}`} 
                      ...
                      activeResource.fileUrl ? (
                        <div className="flex flex-col items-center justify-center h-64 text-gray-400 bg-gray-900 w-full">
                          ...
                          <a 
                            href={`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}${activeResource.fileUrl}`} 
                            target="_blank" 
                            rel="noreferrer"
