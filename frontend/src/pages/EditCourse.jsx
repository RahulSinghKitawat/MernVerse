...
const EditCourse = () => {
  ...
  useEffect(() => {
    const fetchCourseAndQuizzes = async () => {
      try {
        ...
        const quizRes = await axios.get(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/courses/${id}/quizzes`);
  ...
  const removeQuiz = async (quizIndex) => {
    ...
    if (quiz._id) {
      ...
      try {
        await axios.delete(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/quizzes/${quiz._id}`, { headers: { Authorization: `Bearer ${token}` } });
      } catch (err) { 
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/courses/${id}`, courseData, config);

      // Save quizzes sequentially
      for (const quiz of quizzes) {
        if (quiz.title && quiz.questions.length > 0) {
          if (quiz._id) {
            await axios.put(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/quizzes/${quiz._id}`, quiz, config);
          } else {
            await axios.post(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/courses/${id}/quizzes`, quiz, config);
          }
        }
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await axios.delete(`${import.meta.env.VITE_API_URL || "https://mernverse-1.onrender.com"}/api/courses/${id}`, config);
      navigate('/dashboard');
    } catch (err) {
